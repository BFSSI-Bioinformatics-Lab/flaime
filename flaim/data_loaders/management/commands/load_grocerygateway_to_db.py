...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for p in tqdm(j, desc="Loading Grocery Gateway JSON"):
            ...
            # Grocery Gateway fields
            grocerygateway = GroceryGatewayProduct.objects.create(product=product)
            grocerygateway.nutrition_facts_json = p['nutrition']
            if len(p['images']['image_paths']) > 0:
                grocerygateway.image_directory = str(Path(p['images']['image_paths'][0]).parent)
            else:
                grocerygateway.image_directory = None
            grocerygateway.dietary_info = p['Lifestyle & Dietary Need']
            grocerygateway.bullets = p['bullets']
            grocerygateway.sku = p['SKU']
            grocerygateway.changeReason = CHANGE_REASON
            grocerygateway.save()
            ...
            # Serving size does not appear to be available in the Grocery Gateway scrape data
